        ...
        <div className="absolute top-14 left-0 w-48.75 bg-white rounded-b-[11px] shadow-[0px_4px_34px_30px_rgba(0,0,0,0.04)] max-h-60 overflow-y-auto z-10">
          ...
          {filteredOptions.map((option) => (
            <div
              ...
              className={`flex items-center justify-between px-4 py-3 cursor-pointer font-[manrope] text-sm text-[#1B1B1B] transition-colors ${selected.includes(option) ? 'bg-[#FFD15B] font-medium' : 'hover:bg-[#FFD15B]'}`}
            >
              <span>{option}</span>

              {/* Croix visible uniquement si l'option est sélectionnée */}
              {selected.includes(option) && (
                <button
                  ...
                  className="text-[#1B1B1B] hover:text-gray-600"
                >
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="currentColor">
                    <path d="M14 1.41L12.59 0L7 5.59L1.41 0L0 1.41L5.59 7L0 12.59L1.41 14L7 8.41L12.59 14L14 12.59L8.41 7L14 1.41Z" />
                  </svg>
